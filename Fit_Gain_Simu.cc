...
void kolmo()
{
  ...
  std::cout << charge_valeur_fit[om]*0.7 << '\n';
    ...
    for (int eres_count = 1; eres_count < 27; eres_count++) {
      for (int gain_count = 1; gain_count <26; gain_count++) {
        if ((1/(gain_bin_min + gain_bin_width*(gain_count-1))>charge_valeur_fit[om]*0.8) && (1/(gain_bin_min + gain_bin_width*(gain_count-1))<charge_valeur_fit[om]*1.2))
        {
          ...
          TH1D *mc0_full = MC_Tl_208->ProjectionZ("Charge_Tl_208", eres_count, eres_count, gain_count, gain_count);    // first MC histogram
          TH1D *mc1_full = MC_Bi_214->ProjectionZ("Charge_Bi_214", eres_count, eres_count, gain_count, gain_count);    // second MC histogram
          TH1D *mc2_full = MC_K_40->ProjectionZ("Charge_K_40", eres_count, eres_count, gain_count, gain_count);    // second MC histogram
          ...
          if (status == 0) {                       // check on fit status
            ...
            // spectre_om_full->GetYaxis()->SetRangeUser(0, 100000);

            mc0_full->Scale(param1/mc0_full->Integral()*spectre_om_full->Integral());
            mc1_full->Scale(param1/mc1_full->Integral()*spectre_om_full->Integral());
            mc2_full->Scale(param1/mc2_full->Integral()*spectre_om_full->Integral());

            mc0_full->SetLineColor(kGreen);
            mc1_full->SetLineColor(kOrange);
            mc2_full->SetLineColor(kBlack);

            activity_Tl = mc0_full->Integral()/(4*M_PI*1800);
            activity_Bi = mc1_full->Integral()/(4*M_PI*1800);
            activity_K = mc2_full->Integral()/(4*M_PI*1800);
